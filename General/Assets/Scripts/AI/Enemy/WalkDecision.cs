...
public class WalkDecision : Decision
{
    ...
    {
        Collider[] objects = Physics.OverlapSphere(controller.transform.position, controller.stats.attackRange * controller.stats.visionRange, 1<<8);
    ...
    private bool isInVision(StateController controller, Collider collider)
    {
        ...
        if (distance < controller.stats.attackRange * controller.stats.visionRange && angle <= 180)
        {
            return true;
